...
function EventCard({ event, featured = false }: { event: Event, featured?: boolean }) {
    ...
    return (
        <Link 
            ...
            className={`group block bg-brand-cream border border-brand-primary/10 rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 ${cardSize}`}
        >
            <div className={`relative ${imageHeight} overflow-hidden bg-white`}>
                {event.image && (
                    <Image
                ...
                <div className="absolute top-4 right-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide
                        ${event.status === 'upcoming' ? 'bg-brand-primary text-white' : ''}
                        ${event.status === 'ongoing' ? 'bg-green-600 text-white' : ''}
                        ${event.status === 'completed' ? 'bg-neutral-600 text-white' : ''}
                ...
                {featured && event.featured && (
                    <div className="absolute top-4 left-4">
                        <span className="px-3 py-1 bg-brand-accent text-white rounded-full text-xs font-semibold uppercase tracking-wide">
                            Featured
                        </span>
                    </div>
                )}
            ...
            <div className="p-6">
                {event.category && (
                    <p className="text-sm text-brand-primary font-semibold uppercase tracking-wide mb-2">
                ...
                {event.ticketInfo && (
                    <div className="mt-4 pt-4 border-t border-brand-primary/10">
                        {event.ticketInfo.isFree ? (
                            <p className="text-sm font-semibold text-brand-dark mb-2">Free Event</p>
                        ) : (
        ...
        <div className="min-h-screen bg-brand-cream">
            ...
            <section className="bg-gradient-to-r from-brand-primary to-brand-accent text-white py-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <p className="text-xl md:text-2xl text-brand-cream max-w-3xl mx-auto">
            ...
            {featured.length > 0 && (
                <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-3xl font-bold text-brand-dark mb-8">Featured Events</h2>
            ...
            {/* Upcoming Events */}
            <section className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 className="text-3xl font-bold text-brand-dark mb-8">Upcoming Events</h2>
                
                {upcoming.length === 0 ? (
                    <div className="text-center py-12 bg-brand-cream rounded-lg border border-brand-primary/10">
                        <Calendar className="w-16 h-16 text-neutral-300 mx-auto mb-4" />
                        <p className="text-xl text-neutral-600">No upcoming events at the moment.</p>
                        <p className="text-neutral-500 mt-2">Check back soon for exciting new events!</p>
                    </div>
                ) : (
            ...
            {/* Past Events */}
            {past.length > 0 && (
                <section className="py-16 bg-brand-cream border-t border-brand-primary/10">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <h2 className="text-3xl font-bold text-brand-dark mb-8">Past Events</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {past.map((event) => (
                                <EventCard key={event._id} event={event} />
                            ))}
